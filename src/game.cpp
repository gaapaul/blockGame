...
// This inits the entire game takes a time values which is used to scale inputs
// and determine when blocks shall fall
...
// A bag contains block Pieces and which piece shall spawn next
// all pieces are put in bag and shuffled in this function given class a
// shuffled bag.
void game::createBag() {
  int randVal;
  int tmp;
...
// for current level return a drop time.
...
// Run the game logic here, is basically creating a new block and calculating
// FPS. then dropping that block at certain time intervals.
void game::runGame(GLFWwindow *window,
  ...
  if (diff.count() > this->calcDropTime()) {
    ...
    if (this->checkCollision(&tetronimo)) {
      ...
      if (freezeDiff.count() > this->freezeBlockTime ||
          this->maxRotateBeforeFreeze > 4) {
        this->freezeBlock();
      }
    } else {
...
void game::renderGame() {
  ...
  std::cout << "NEXT: " << this->tetronimoBag[this->positionInBag + 1]
            << std::endl;
  ...
  this->textRenderer.renderText(std::to_string(this->fps),
                                glm::vec2(0.0f, 0.2f), glm::vec2(.5, .5),
                                glm::vec3(1.0, 0.0, 0.0));
  this->textRenderer.renderText(std::to_string(this->visualScore),
                                glm::vec2(-.25f, 0.2f), glm::vec2(.5, .5),
                                glm::vec3(1.0, 0.0, 0.0));
  this->textRenderer.renderText("LEVEL", glm::vec2(-.35f, 0.0f),
                                glm::vec2(.5, .5), glm::vec3(1.0, 0.0, 0.0));
  this->textRenderer.renderText(std::to_string(this->level),
                                glm::vec2(-0.1f, 0.0f), glm::vec2(.5, .5),
                                glm::vec3(1.0, 0.0, 0.0));
  this->textRenderer.renderText("LINES", glm::vec2(-.35f, -0.2f),
                                glm::vec2(0.5, .5), glm::vec3(1.0, 0.0, 0.0));
  this->textRenderer.renderText(std::to_string(this->lines),
                                glm::vec2(-0.1f, -0.2f), glm::vec2(.5, .5),
                                glm::vec3(1.0, 0.0, 0.0));
}
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (piece->curr_block[j][i].tIsRendered &&
          (board.tilemap[i + (int)piece->xPos][j + (int)piece->yPos] != 0 &&
           board.tilemap[i + (int)piece->xPos][j + (int)piece->yPos] != 10)) {
        return true;
      }
...
int game::keyboardInput(GLFWwindow *window,
                        std::chrono::steady_clock::time_point time) {
  int tmp;
  ...
  if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS &&
      this->inputKeys[KEY_D]->keyPressed(time)) {
  ...
  if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS &&
      this->inputKeys[KEY_A]->keyPressed(time)) {
    this->tetronimo.xPos -= 1;
    // this->checkCollision();
    if (this->checkCollision(&tetronimo))
      this->tetronimo.xPos += 1;
  ...
  if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS &&
      this->inputKeys[KEY_W]->keyPressed(time)) {
  ...
  if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS &&
      this->inputKeys[KEY_S]->keyPressed(time)) {
    this->tetronimo.yPos -= 1;
    // this->checkCollision();
    if (this->checkCollision(&tetronimo)) {
      this->tetronimo.yPos += 1;
  ...
  if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS &&
      this->inputKeys[KEY_SPACE]->keyPressed(time)) {
    // this->checkCollision();
    int score = 0;
  ...
  if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS &&
      this->inputKeys[KEY_Q]->keyPressed(time)) {
  ...
  if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS &&
      this->inputKeys[KEY_E]->keyPressed(time)) {
