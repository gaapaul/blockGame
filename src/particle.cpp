...
// draws particles and destroys them if out lines life span
...
// Create partiles at a position with a density defined in init and a velocity
// and color
void particleGen::createParticles(float yPosition, float xPosition, int color) {
  ...
  newParticle.particleSprite =
      tile(tileTexture,
           glm::vec2((float)xPosition / 20 + this->board_offset_x,
                     (float)yPosition / 20 + this->board_offset_y),
           this->tileSize, 0, color);
...
// reduces partile lifespan and updates position on velocity
